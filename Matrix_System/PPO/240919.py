...
class PPO:
    def __init__(self, state_dim, action_dim, max_training_steps):
        ...
        # ActorCritic 네트워크
        self.policy = ActorCritic(state_dim, action_dim, 1024)
    ...
    def select_action(self, state):
        ...
        action, action_logprob, state_value = self.policy.select_action(state)
        return action, action_logprob, state_value
    
    def compute_returns_and_advantages(self, rewards, state_values, is_terminals, gamma, device):
    ...
    def update(self):
        ...
        # 할인된 리턴 및 Advantage 계산
        returns, advantages = self.compute_returns_and_advantages(rewards, state_values, is_terminals, self.gamma, states.device)
...
for i in range(agent.num_episodes):
    ...
    if i % 100 == 0 or i == agent.num_episodes-1:
